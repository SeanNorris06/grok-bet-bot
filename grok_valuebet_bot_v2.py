"""
GROK VALUEBET BOT V4.2 - VERSION PLUS GÉNÉREUSE
Ultra Safe (très strict) + Picks Intéressants (beaucoup plus de pronos)
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔥 *Grok ValueBet V4.2 est prêt !*\nEnvoi automatique à 10h30 et 14h30.\nTape /today pour tester maintenant.")

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📋 *Commandes :*\n\n"
        "/today → Pronostics immédiats\n"
        "/help → Cette aide\n"
        "/stats → Infos rapides\n\n"
        "Le bot envoie chaque jour les meilleurs paris foot."
    )
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"📊 Bankroll : {BANKROLL}€\nMode : Ultra Safe + Picks Intéressants\nEnvoi auto 10h30 / 14h30")
...
def calculate_picks(match):
    ...
    # Picks Intéressants (plus généreux)
    if h2h.get("Home"):
        dc_1x = p_home + p_draw
        odds = round(1 / dc_1x * 0.93, 2)
        edge = round((dc_1x - 1/odds) * 100, 1)
        if edge >= 3 and dc_1x >= 0.53:
...
def run_analysis():
    ...
    msg = f"🔥 **GROK VALUEBET V4.2** - {datetime.datetime.now(pytz.timezone('Europe/Paris')).strftime('%d/%m %H:%M')}\n"
    msg += f"💰 Bankroll : {BANKROLL:.0f}€\n\n"
    
    ultra_safe = [r for r in data if r[4] == "Ultra Safe"][:5]
    interesting = [r for r in data if r[4] == "Intéressant"][:10]
    ...
    msg += "\n💎 **PICKS INTÉRESSANTS** (bons paris jouables)\n"
    for r in interesting:
        msg += f"✅ {r[3]} → {r[5]} @ {r[6]} (edge +{r[7]}%)\n"
    
    if not interesting:
        msg += "😴 Aucun pari intéressant trouvé aujourd'hui. Reviens demain !"
    
    send_message(msg)
    print("✅ Message V4.2 envoyé !")
...
def main():
    ...
    print("✅ GROK VALUEBET V4.2 lancée !")
    app.run_polling()
